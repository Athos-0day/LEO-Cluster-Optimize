...
int main() {
  ...
  std::string pathUser1 = "../data/UserAndId.csv";
  std::string pathUser2 = "../data/UserAndIdFin.csv";
  double proportion = 0.8;
  std::vector<UserPoint> users = CsvToUser(path, false, "");

  // creation des deux listes d'utlisateurs :
  one_to_tow(users, pathUser1, pathUser2, proportion);

  std::vector<UserPoint> usersInitiaux = CsvIdToUser(pathUser1);
  std::vector<UserPoint> usersRajout = CsvIdToUser(pathUser2);

  if (usersInitiaux.empty()) {
  ...
  // 1. --- Test de l'algorithme Classique (Greedy) --- Desactive car trop long
  ...
  for (const auto &config : q_configs) {
    for (size_t s = 0; s < strategies.size(); ++s) {
      for (int fill : taux_remplissage) {
        ...
        auto start = std::chrono::high_resolution_clock::now();
        clusters = runQuadtreeClustering(
            usersInitiaux, strategies[s], fill, config.strat, config.global_mean, clusters);
        auto end = std::chrono::high_resolution_clock::now();
        std::chrono::duration<double, std::milli> duration = end - start;
        ...
        std::cout << std::left << std::setw(12) << "Quadtree utilisateurs initiaux" << std::setw(12)
                  << str_names[s] << std::setw(6) << fill << std::setw(15)
        ...
        // on complete les clusters

        start = std::chrono::high_resolution_clock::now();
        clusters = runQuadtreeClustering(
            usersRajout, strategies[s], fill, config.strat, config.global_mean, clusters);
        ...
        std::cout << std::left << std::setw(12) << "Quadtree utilisateurs Rajoute" << std::setw(12)
                  << str_names[s] << std::setw(6) << fill << std::setw(15)
                  << config.name << std::setw(10) << metrics.nb_clusters
                  << std::fixed << std::setprecision(2) << std::setw(12)
                  << metrics.avg_fill_rate_percent << std::setw(12)
