...
int main() {
  ...
  std::vector<std::string> str_names = {"Pessimiste", "Optimiste", "Overbooking"};
  std::vector<int> taux_remplissage = {80, 100};
  std::vector<bool> hilbert_options = {false, true};
  std::vector<bool> merge_options = {false, true};  // NOUVEAU
  ...
  std::cout << std::left
            << std::setw(10) << "Algo"
            << std::setw(12) << "Stratégie"
            << std::setw(6)  << "Cap%"
            << std::setw(13) << "Post-Trait"
            << std::setw(8)  << "Hilbert"
            << std::setw(8)  << "Merge"      // NOUVELLE COLONNE
            << std::setw(10) << "Clusters"
            << std::setw(10) << "Rempli(%)"
            << std::setw(13) << "Dist_Moy(km)"
            << std::setw(10) << "Mem(Ko)"
            << std::setw(10) << "Temps(ms)"
            << std::endl;
  std::cout << std::string(114, '-') << std::endl;

  for (bool use_merge : merge_options) {
          ...
          for (int fill : taux_remplissage) {
            ...
            std::vector<Cluster> clusters =
                runQuadtreeClustering(users, strategies[s], fill, config.strat,
                                      config.global_mean, use_hilbert, use_merge);
            auto end = std::chrono::high_resolution_clock::now();
            std::chrono::duration<double, std::milli> duration = end - start;

            ClusteringMetrics metrics = computeMetrics(clusters, users, capacite_cible_mbps);

            std::cout << std::left
                      << std::setw(10) << "Quadtree"
                      << std::setw(12) << str_names[s]
                      << std::setw(6)  << fill
                      << std::setw(13) << config.name
                      << std::setw(8)  << (use_hilbert ? "OUI" : "NON")
                      << std::setw(8)  << (use_merge   ? "OUI" : "NON")  // NOUVEAU
                      << std::setw(10) << metrics.nb_clusters
                      << std::fixed << std::setprecision(2)
                      << std::setw(10) << metrics.avg_fill_rate_percent
                      << std::setw(13) << metrics.avg_distance_km
                      << std::setw(10) << metrics.memory_footprint_kb
